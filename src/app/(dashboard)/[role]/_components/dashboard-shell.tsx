...
const SEGMENT_QUERY_STALE: Record<
  string,
  {
...
> = {
  ...
  "activity-logs": {
    keys: (uid) => [["activity-logs", uid]],
    staleMs: () => ACTIVITY_LOGS_STALE_TIME,
  },
};
...
export function DashboardShell({ children, initialUser }: DashboardShellProps) {
  ...
  // Seed initial user data from the server-rendered user so the header
  // shows the right name immediately on first paint (no flash).
  const [userData, setUserData] = useState<UserData>({
    name:
      `${initialUser.first_name ?? ""} ${initialUser.last_name ?? ""}`.trim() ||
      initialUser.username ||
      "User",
    role: initialUser.role || pathRole,
  });
  ...
  useEffect(() => {
    try {
      if (user) {
        const roleValue =
          typeof user.role === "string" ? parseInt(user.role, 10) : Number(user.role);
  ...
  // Defense-in-depth role guard — server already redirected on role-segment
  // mismatch, but this also catches in-role page-permission violations
  // (e.g. staff trying to load /staff/accounts).
  useEffect(() => {
    ...
    const roleAllowed = allowedPages[actualRole] ?? [
      "calendar",
      "reservations",
      "reservation-tracking",
    ];
    ...
    if (
      pageSegment &&
      !universalPages.includes(pageSegment) &&
      !roleAllowed.includes(pageSegment)
    ) {
      const defaultPage = getDefaultPageForRole(actualRole);
  ...
  useLayoutEffect(() => {
    if (prevPathname.current !== null && prevPathname.current !== pathname) {
      ...
      const fresh =
        !forceOverlay && hasCachedFreshData(queryClient, pageSegment, userId, role);
      if (fresh) {
  ...
  // 300ms floor for first-visit loads so the spinner is visible long enough.
  useEffect(() => {
