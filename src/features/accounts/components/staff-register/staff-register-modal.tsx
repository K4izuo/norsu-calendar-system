...
import { Briefcase, X, ArrowLeft, ArrowRight, SendHorizontal } from "lucide-react";
...
import { staffSchema, staffWithCredentialsSchema, StaffWithCredentialsFormData } from "@/features/auth/utils/staff/staff-register-validation-rules";
...
export function StaffRegisterModal({ isOpen, onClose }: StaffRegisterModalProps) {
  ...
  const { control, handleSubmit, watch, register, reset, trigger, setValue, getValues, formState: { errors, isSubmitting, isValid } } =
    useForm<StaffWithCredentialsFormData>({
      resolver: zodResolver(staffWithCredentialsSchema),
      mode: "onTouched",
      defaultValues: {
        first_name: "",
        middle_name: "",
  ...
  const onSubmit = useCallback(
    async (data: StaffWithCredentialsFormData) => {
      try {
        ...
        toast.success("Staff registration successful!", { duration: 5000 });
        reset();
  ...
  const handleNext = useCallback(async () => {
    ...
    if (!valid) {
      fields.forEach((f) => setValue(f, getValues(f), { shouldTouch: true, shouldValidate: false }));
      return;
    }
          ...
          <div className="sticky top-0 bg-white z-10 p-4 sm:p-6 pb-4 sm:pb-6 border-b border-gray-200">
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-2">
                <Briefcase strokeWidth={2.5} className="w-8 h-8 text-gray-800 shrink-0" />
