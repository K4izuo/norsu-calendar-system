...
const CONTENT_X = 114;            // exact match to template label x=113.78
const CONTENT_RIGHT = 585;            // right edge of content
const CONTENT_W = CONTENT_RIGHT - CONTENT_X; // 471 pt
const FS = 10;             // font size
const LINE_H = 14.5;             // baseline-to-baseline (matches template 14.5pt row spacing)
...
class Canvas {
  ...
  py(fromTop: number): number { return this.h - fromTop; }
  ...
  // Regular text, wrapped, optional left indent
  text(content: string, indent = 0) {
    const x = CONTENT_X + indent;
    const avail = CONTENT_W - indent;
    const words = content.split(" ");
    let line = "";
    const flush = () => { this.draw(line, x, this.regular); this.nl(); };
    for (const word of words) {
      const c = line ? `${line} ${word}` : word;
      if (this.regular.widthOfTextAtSize(c, FS) > avail && line) { flush(); line = word; }
      else line = c;
    }
    if (line) flush();
  }
  ...
  equipTable(items: { name: string; quantity: number }[]) {
    ...
    const ROW_H = 17;
    const col1W = Math.round(CONTENT_W * 0.70);
    const totalH = ROW_H * (items.length + 1); // +1 for header
    const top = this.y;

    // Gray header background (drawn before borders so borders are on top)
    this.page.drawRectangle({
      x: CONTENT_X, y: this.py(top + ROW_H),
    ...
    for (let r = 1; r <= items.length; r++) {
      ...
      this.page.drawLine({ start: { x: CONTENT_X, y: ly }, end: { x: CONTENT_X + CONTENT_W, y: ly }, thickness: 0.5, color: BLACK });
    }

    // Vertical column separator
    const vx = CONTENT_X + col1W;
    this.page.drawLine({ start: { x: vx, y: this.py(top) }, end: { x: vx, y: this.py(top + totalH) }, thickness: 0.5, color: BLACK });

    // Header labels
    this.page.drawText("Equipment type", { x: CONTENT_X + 4, y: this.py(top + ROW_H - 5), size: FS, font: this.bold, color: BLACK });
    this.page.drawText("Quantity", { x: vx + 4, y: this.py(top + ROW_H - 5), size: FS, font: this.bold, color: BLACK });
    ...
    items.forEach((item, i) => {
      ...
      this.page.drawText(String(item.quantity), { x: vx + 4, y: ty, size: FS, font: this.regular, color: BLACK });
    });
  ...
  printedBy(name: string) {
    ...
    this.page.drawLine({ start: { x: lx, y: baselineY }, end: { x: lx + 160, y: baselineY }, thickness: 0.5, color: BLACK });
...
export async function printEventReceipt(
  ...
  cv.text(event.description || "N/A", 15);
  cv.gap(LINE_H);
